/* ═══════════════════════════════════════
   KALORYX by ABG — App Logic v2.1
   Fixes: session persistence, logout, profile save, auth flow
═══════════════════════════════════════ */
...
const sb = supabase.createClient(SUPABASE_URL, SUPABASE_KEY, {
  auth: {
    ...
    detectSessionInUrl:true    // ← catches Google OAuth redirect
  }
});
...
function showAuth() {
  ...
  const p2 = document.getElementById('auth-pass2');
  if (p2) p2.value = '';
...
async function doAuth() {
  ...
  try {
    if (authMode === 'register') {
      ...
      okEl.textContent   = '¡Cuenta creada! Revisa tu correo para confirmar tu cuenta.';
    ...
    if (msg.includes('Email not confirmed')) msg = 'Debes confirmar tu correo antes de iniciar sesión.';
...
/* ══════════════════════════════
   SESSION INIT (runs on load)
══════════════════════════════ */
async function initSession() {
  applyTheme();

  // 1. Check for existing session (handles page reload)
  ...
  // 2. Listen for auth changes (login, Google OAuth redirect, logout)
...
async function addFoodDB(food) {
  ...
  if (!error) {
    ...
    console.error('addFoodDB error:', error);
...
Alimento: "${desc}"`
    ...
    aiPending = {
      name:   j.nombre || desc,
      cal:    j.kcal,
      moment: document.getElementById('ai-moment').value
    };
...
async function analyzePhoto() {
  ...
  try {
    ...
    photoPending = {
      name:   j.nombre,
      cal:    j.kcal,
      moment: document.getElementById('photo-moment').value
    };
...
async function addWorkout() {
  ...
  ['w-name','w-hrs','w-mins','w-cal','w-notes'].forEach(id => {
...
function recalcBMR() {
  ...
  const metaInput = document.getElementById('p-calmeta');
  if (tdee && !metaInput.value)
    metaInput.placeholder = 'Sugerido: ' + tdee + ' kcal';
}
...
async function saveProfile() {
  ...
  // Build object — only include non-empty values to avoid DB type errors
  const raw = {
  ...
  // Remove null keys that Supabase might reject
...
async function logWeight() {
  ...
  const { error } = await sb.from('weight_log')
    .upsert(
      { user_id: USER.id, weight: w, logged_at: today },
  ...
  if (!error) {
    ...
    alert('Error al registrar peso: ' + error.message);
  }
}
